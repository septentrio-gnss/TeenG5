...
The mosaicG5 HAT has 3 options for powering the board; Raspberry Pi, USB-C and external power pin headers. The mosaic-G5 module itself runs on 3.3V, thus a buck converter(MP2145GD-Z) is used to regulate the voltage from 5V to 3.7V and an LDO voltage regulator is used to filter the switching noise from the buck converter and to regulate the voltage from 3.6 to 3.3volts(TPS7A9401DSCR). Raspberry Pi and USB-C already provide 5V.
